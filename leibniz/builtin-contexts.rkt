...
(define-context IEEE-floating-point-with-conversion
  ...
  (include rational-numbers)
  (op (ℤ->IEEE-binary32 ℤ) IEEE-binary32)
  (op (ℤ->IEEE-binary64 ℤ) IEEE-binary64)
  (op (ℚ->IEEE-binary32 ℤ) IEEE-binary32)
  (op (ℚ->IEEE-binary64 ℤ) IEEE-binary64)
  (op (IEEE-binary32->ℚ IEEE-binary32-number) ℚ)
  (op (IEEE-binary64->ℚ IEEE-binary64-number) ℚ)
  (-> #:vars ([X ℤ])
      (ℤ->IEEE-binary32 X) (unary-op integer? real->single-flonum))
  (-> #:vars ([X ℤ])
      (ℤ->IEEE-binary64 X) (unary-op integer? real->double-flonum))
  (-> #:vars ([X ℚ])
      (ℚ->IEEE-binary32 X) (unary-op exact? real->single-flonum))
  (-> #:vars ([X ℚ])
      (ℚ->IEEE-binary64 X) (unary-op exact? real->double-flonum))
  (-> #:vars ([X IEEE-binary32-number])
      (IEEE-binary32->ℚ X) (unary-op single-flonum? inexact->exact))
  (-> #:vars ([X IEEE-binary64-number])
      (IEEE-binary64->ℚ X) (unary-op double-flonum? inexact->exact)))

(module+ test
  (with-context IEEE-floating-point-with-conversion
    (chk
     #:= (RT (ℤ->IEEE-binary32 1)) (T #x1s0)
     #:= (RT (ℤ->IEEE-binary64 1)) (T #x1l0)
     #:= (RT (ℚ->IEEE-binary32 1/2)) (T #x0.8s0)
     #:= (RT (ℚ->IEEE-binary64 1/2)) (T #x0.8l0)
     #:= (RT (IEEE-binary32->ℚ #x0.8s0)) (T 1/2)
     #:= (RT (IEEE-binary64->ℚ #x0.8l0)) (T 1/2)
     #:= (RT (IEEE-binary32->ℚ +nan.f)) (T (IEEE-binary32->ℚ +nan.f))
     #:= (RT (IEEE-binary64->ℚ +inf.0)) (T (IEEE-binary64->ℚ +inf.0)))))
